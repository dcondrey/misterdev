...
from misterdev.core.execution.escalation import RUNGS, choose_rung, should_count_failure


def test_ladder_climbs_with_code_failures():
    assert choose_rung(0) == "normal"
    assert choose_rung(1) == "widen_context"
    assert choose_rung(2) == "full_rewrite"
    assert choose_rung(3) == "decompose"
    assert choose_rung(9) == "decompose"  # decompose is terminal


def test_ladder_has_a_structural_full_rewrite_rung():
    # T3.1: a real, structurally-different rung sits between the context-widening
    # reprompt and the model swap.
    assert "full_rewrite" in RUNGS
    assert (
        RUNGS.index("widen_context")
        < RUNGS.index("full_rewrite")
        < RUNGS.index("stronger_model")
    )
    assert choose_rung(2) == "full_rewrite"


def test_custom_thresholds():
    kw = dict(widen_after=2, rewrite_after=3, model_after=4, decompose_after=6)
    assert choose_rung(1, **kw) == "normal"
    assert choose_rung(2, **kw) == "widen_context"
    assert choose_rung(3, **kw) == "full_rewrite"
...
def test_infra_failures_never_advance_the_ladder():
    ...
    outputs = [
        ...
        "AssertionError: 1 != 2",  # code   -> full_rewrite
        "operation timed out",  # infra  -> hold at full_rewrite
